...
2. **词表裁剪按目标全集、可逆**：全词表 248320 仅嵌入≈0.5B，是预算主负担。裁到 ~110k，第一版只训中英数据。裁剪可逆（存 id 映射 + 保留原始 embedding），扩语言时只补数据不动结构。总预算 ~0.7B，transformer ≈0.47B。
   - **keep 集选择铁律（2026-06-02 修正，曾因此踩坑）**：频次必须统计**真实训练语料**（train jsonl 的 src+tgt 双侧），且**训练语料用到的 token 全部强制保留**（OOV 必须≈0）；预留语言只用**剩余**预算填，绝不挤占中英。
   - 教训：旧版用"多语言校准语料"统计频次 + 预留语言占额，导致 23589 个中文常用合并 token 被裁、en2zh 目标 12% OOV，而 54% 预算浪费在语料用不到的 token 上 → 过早 eos/空输出（详见 docs/paper/01-worklog.md §4.5）。
   - **OOV 兜底不得映射成 eos**（会污染 eos 标签、训出早停）；映射到非破坏性 token（如 pad/空格）或在 keep 集留专用占位。
