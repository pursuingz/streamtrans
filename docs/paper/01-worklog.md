...
### 4.4 12000-step（≈4 epoch）实验（进行中）

- 设置：12000 step，batch=32（有效 batch 128），每 3000 step 存中间 checkpoint（3k/6k/9k/12k）。
- 目的：一趟训练得**质量-epoch 曲线**，定位质量饱和点，免反复重训。
- 推理侧叠加 `repetition_penalty=1.3` + `no_repeat_ngram_size=3` 压退化死循环。
- 结果待回填。
